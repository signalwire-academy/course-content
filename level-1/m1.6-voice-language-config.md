...
Fillers are phrases spoken while the AI is processing or waiting:
...
Set fillers for specific functions that take time:
...
Keep it to 3-5 appropriate fillers.
...
```python
# Professional prompt but casual speech_fillers
agent.prompt_add_section("Role", "You are a formal legal assistant.")
agent.add_language("English", "en-US", "rime.spore",
    speech_fillers=["Sure thing!", "No prob!", "Gotcha!"])
```
