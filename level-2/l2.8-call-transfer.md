...
class ReceptionistAgent(AgentBase):
    ...
    def __init__(self):
        ...
        self.prompt_add_section(
            "Role",
            "You are the receptionist. Route calls to departments."
        )
    ...
    def _is_department_open(self, department: str) -> tuple:
        ...
        if start <= hour < end:
            return True, None
        return False, f"{department.title()} hours: {start}:00-{end}:00"

    def _setup_functions(self):
        @self.tool(description="List departments")
        ...
        @self.tool(description="Check availability", parameters={
            "type": "object",
        ...
        def check_availability(args: dict, raw_data: dict = None) -> SwaigFunctionResult:
            ...
            is_open, message = self._is_department_open(department)
            if is_open:
                return SwaigFunctionResult(f"{department.title()} is open now.")
            return SwaigFunctionResult(message)

        @self.tool(description="Transfer with context", parameters={
            "type": "object",
            "properties": {
                "department": {"type": "string", "enum": list(self.DEPARTMENTS.keys())},
                "reason": {"type": "string"},
